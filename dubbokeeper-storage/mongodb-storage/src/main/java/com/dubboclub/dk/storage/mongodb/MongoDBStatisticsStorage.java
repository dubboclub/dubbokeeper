...
public class MongoDBStatisticsStorage implements StatisticsStorage, InitializingBean {
	...
	public void storeStatistics(Statistics statistics) {
		if (!APPLICATION_STORAGES.containsKey(statistics.getApplication().toLowerCase())) {
			ApplicationStatisticsStorage applicationStatisticsStorage = new ApplicationStatisticsStorage(applicationDao,
					statisticsDao, statistics.getApplication(), true);
			ApplicationStatisticsStorage old = APPLICATION_STORAGES
					.putIfAbsent(statistics.getApplication().toLowerCase(), applicationStatisticsStorage);
			if (old == null) {
	...
	public Collection<MethodMonitorOverview> queryMethodMonitorOverview(String application, String serviceInterface,
		...
		for (TempMethodOveride method : methods) {
			...
			Statistics concurrent = statisticsDao.findMethodMaxItemByService("concurrent", application,
					serviceInterface, method.getM(), startTime, endTime);
			Statistics elapsed = statisticsDao.findMethodMaxItemByService("elapsed", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics failure = statisticsDao.findMethodMaxItemByService("failureCount", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics input = statisticsDao.findMethodMaxItemByService("input", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics kbps = statisticsDao.findMethodMaxItemByService("kbps", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics output = statisticsDao.findMethodMaxItemByService("output", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics success = statisticsDao.findMethodMaxItemByService("successCount", application, serviceInterface,
					method.getM(), startTime, endTime);
			Statistics tps = statisticsDao.findMethodMaxItemByService("tps", application, serviceInterface,
					method.getM(), startTime, endTime);
	...
	public Collection<ApplicationInfo> queryApplications() {
		...
		for (ApplicationInfo applicationInfo : applicationInfos) {
			ApplicationStatisticsStorage applicationStatisticsStorage = APPLICATION_STORAGES
					.get(applicationInfo.getApplicationName());
	...
	public ApplicationInfo queryApplicationInfo(String application, long start, long end) {
		...
		applicationInfo.setApplicationType(getAppType(applicationInfo.getApplicationName()));
		...
		applicationInfo.setApplicationType(getAppType(applicationInfo.getApplicationName()));
		stopWatch.stop();
		LOGGER.info(String.format("Method:%s Time:%s Param:%s,%s,%s", "queryApplicationInfo", stopWatch.getTime(),
	...
	public StatisticsOverview queryServiceOverview(String application, String service, long start, long end) {
		StatisticsOverview statisticsOverview = new StatisticsOverview();
		List<Statistics> statisticses = statisticsDao.findServiceOverview(application, service, "concurrent", start,
				end);
		fillConcurrentItem(statisticses, statisticsOverview);
	...
	public Collection<ServiceInfo> queryServiceByApp(String application, long start, long end) {
		...
		stopWatch.stop();
		LOGGER.info(String.format("Method:%s Time:%s Param:%s,%s,%s", "queryServiceByApp", stopWatch.getTime(),
				application, start, end));
		return infos;
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		for (ApplicationInfo app : apps) {
			ApplicationStatisticsStorage applicationStatisticsStorage = new ApplicationStatisticsStorage(applicationDao,
					statisticsDao, app.getApplicationName());
	...
	private Integer getAppType(String applicationName) {
		boolean provider = false;
		boolean consumer = false;
		List<ServiceInfo> infos = statisticsDao.findServiceByApp(applicationName);
		for (ServiceInfo service : infos) {
			if ("PROVIDER".equals(service.getRemoteType())) {
				provider = true;
			} else if ("CONSUMER".equals(service.getRemoteType())) {
				consumer = true;
			}
